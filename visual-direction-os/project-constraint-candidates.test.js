...
function baseFixture() {
  ...
  const projectIntelligence = {
    schemaVersion:'0.1.0', mode:'shadow', status:'UNRESOLVED',
    ...
    boundaries:[
      {
        id:'scene-01->scene-02',fromSceneId:'scene-01',toSceneId:'scene-02',status:'PASS',rule:'boundary-pass',evidenceStatus:'supported',
        visualResponse:[
          {family:'camera',from:'WORLD',to:'CONTESTED',changed:true,source:'compiler-backed'},
          {family:'color',from:'WORLD',to:'WORLD',changed:false,source:'ai-completed'}
        ]
      },
...
test('derives immediate-next SETUP Camera carry from material compiler-backed evidence', () => {
  ...
  const candidates = candidatesApi.deriveProjectConstraintCandidates(input);
  assert.equal(candidates.length, 1);
  assert.equal(candidates[0].type, 'ownership-carry');
  assert.equal(candidates[0].sourceBoundaryId, 'scene-01->scene-02');
  assert.equal(candidates[0].sourceSceneId, 'scene-02');
  assert.equal(candidates[0].targetSceneId, 'scene-03');
  ...
  assert.equal(candidates[0].evidence.source, 'compiler-backed');
  assert.deepEqual(input, before, 'Candidate derivation must not mutate inputs');
  assert.deepEqual(candidatesApi.deriveProjectConstraintCandidates(input), candidates, 'Candidate derivation must be deterministic');
});

test('exact expected value comes from final Scene State, not normalized M6 authority label', () => {
  const input = baseFixture();
  input.projectIntelligence.scenes[1].cameraAuthority = 'CONTESTED';
  input.projectState.scenes['scene-02'].workspace.sceneState.variables.camera.perspective = 'mixed';
  const [candidate] = candidatesApi.deriveProjectConstraintCandidates(input);
  assert.equal(candidate.expected, 'mixed');
});

test('AI-completed, legacy, unknown/divergent, blocked, WARN, and handoff mismatch are ineligible', () => {
  const cases = [
    input => { input.projectIntelligence.scenes[1].sources.camera = 'ai-completed'; },
    input => { input.projectIntelligence.scenes[1].provenanceStatus = 'legacy'; },
    input => { input.projectIntelligence.scenes[1].sources.camera = 'unknown'; input.projectIntelligence.scenes[1].integrityFindings = [{rule:'provenance-final-state-divergence'}]; },
    input => { input.projectIntelligence.scenes[1].sources.camera = 'blocked'; },
    input => { input.projectIntelligence.boundaries[0].status = 'WARN'; },
    input => { input.projectIntelligence.scenes[2].narrativeAgency.start = 'CHARACTER'; }
  ];
  for (const mutate of cases) {
    const input = baseFixture(); mutate(input);
    assert.deepEqual(candidatesApi.deriveProjectConstraintCandidates(input), []);
  }
});

test('does not skip a directed immediate target to constrain a later Scene', () => {
  const input = baseFixture();
  input.projectState.scenes['scene-03'].status.visual = 'directed';
  input.projectState.sceneOrder.push('scene-04');
  input.projectState.scenes['scene-04'] = clone(input.projectState.scenes['scene-03']);
  input.projectState.scenes['scene-04'].id = 'scene-04';
  input.projectState.scenes['scene-04'].status.visual = 'undirected';
  input.projectIntelligence.sceneOrder.push('scene-04');
  input.projectIntelligence.scenes.push({sceneId:'scene-04',provenanceStatus:'missing',narrativeRole:'RESOLUTION',narrativeAgency:{start:'CONTESTED',end:'CHARACTER'},sources:{camera:'unknown',color:'unknown'},integrityFindings:[]});
  assert.deepEqual(candidatesApi.deriveProjectConstraintCandidates(input), []);
});

test('identical rejected Candidate stays hidden until compatible material evidence changes', () => {
  const input = baseFixture();
  const first = candidatesApi.deriveProjectConstraintCandidates(input)[0];
  const dismissed = registry.rejectCandidate(registry.createEmptyRegistry(), first);
  assert.deepEqual(candidatesApi.deriveProjectConstraintCandidates({...input,registry:dismissed}), []);

  const changed = clone(input.projectState);
  changed.scenes['scene-03'].narrativeRole.agencyTransition = ['contested','shared','character'];
  const changedIntel = clone(input.projectIntelligence);
  changedIntel.scenes[2].narrativeAgency = {start:'CONTESTED',end:'CHARACTER'};
  const next = candidatesApi.deriveProjectConstraintCandidates({projectState:changed,projectIntelligence:changedIntel,registry:dismissed});
