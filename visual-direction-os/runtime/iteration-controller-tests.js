...
const artifact = {
  id: 'gen-iteration-1',
  request: { model: 'agnes-image-2.1-flash', prompt: 'BASE\n\nOLD DELTA', size: '2K', ratio: '16:9', return_base64: true, extra_body: { response_format: 'b64_json' } },
  baseRequest: { model: 'agnes-image-2.1-flash', prompt: 'BASE', size: '2K', ratio: '16:9', return_base64: true, extra_body: { response_format: 'b64_json' } },
...
(async () => {
  ...
  assert.equal(calls[0][1].prompt, `BASE\n\n${delta.promptAppendix}`, 'iteration must compile from clean baseRequest, not append onto OLD DELTA');
  assert.doesNotMatch(calls[0][1].prompt, /OLD DELTA/);
  ...
  assert.deepEqual(calls[1][2].baseRequest, artifact.baseRequest);
  assert.notEqual(calls[1][2].baseRequest, artifact.baseRequest, 'generation context should snapshot the base request');
  ...
  calls.length = 0;
  const memoryAppendix = 'ITERATION / DIRECTOR MEMORY\n\nPRESERVE LOCKED:\n- Keep canvas\n\nCORRECT ACTIVE:\n- Reduce density';
  await runGenerationIteration({
    root,
    artifact,
    delta,
    promptAppendix:memoryAppendix,
    applyIterationDelta
  });
  assert.equal(calls[1][1].prompt, `BASE\n\n${memoryAppendix}`, 'M4 promptAppendix override should replace the M3 delta appendix for this iteration');
  assert.deepEqual(calls[1][2].baseRequest, artifact.baseRequest);

  const alternateBase = { ...artifact.baseRequest, prompt:'ALTERNATE BASE' };
  calls.length = 0;
  await runGenerationIteration({ root, artifact, delta, baseRequest:alternateBase, applyIterationDelta });
  assert.equal(calls[1][1].prompt, `ALTERNATE BASE\n\n${delta.promptAppendix}`);
  assert.deepEqual(calls[1][2].baseRequest, alternateBase);
