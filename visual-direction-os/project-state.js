((root, factory) => {
  ...
  const constraintRegistry = typeof module === 'object' && module.exports
    ...
    : null;
  const api = factory(root, contracts, constraintRegistry);
  if (typeof module === 'object' && module.exports) module.exports = api;
  if (root) root.VDOSProjectState = api;
})(typeof window !== 'undefined' ? window : globalThis, (root, contracts, constraintRegistry) => {
  'use strict';
  ...
  function createProjectStore(initial = null) {
    ...
    function getProject() { return clone(state); }
    function requireProject() { if (!state) throw new Error('Create a Project before modifying it.'); }
    ...
    function normalizeOrders(project) {
      project.sceneOrder.forEach((id, index) => { project.scenes[id].order = index + 1; });
      return project;
    }
    ...
    function setProjectConstraints(nextRegistry) {
      ...
      if (!registryApi?.validateRegistry) throw new Error('Invalid Project Constraint Registry: registry validator unavailable');
      const checked = registryApi.validateRegistry(nextRegistry);
    ...
    function addScene(sceneInput = {}, options = {}) {
      requireProject();
      const next = getProject();
      const index = Number.isInteger(options.index) ? Math.max(0, Math.min(options.index, next.sceneOrder.length)) : next.sceneOrder.length;
      ...
      const record = sceneInput.workspace && sceneInput.status ? clone(sceneInput) : createEmptySceneRecord({
        id: generatedId, order: index + 1, title: sceneInput.title,
        narrativeRole: sceneInput.narrativeRole || {
          role: sceneInput.role, narrativeFunction: sceneInput.narrativeFunction,
          startingState: sceneInput.startingState, endingState: sceneInput.endingState,
          turningPoint: sceneInput.turningPoint, agencyTransition: sceneInput.agencyTransition,
    ...
    function reorderScenes(sceneIds) {
      ...
      const next = getProject(); next.sceneOrder = clone(sceneIds); normalizeOrders(next);
      return validateAndCommit(next, 'scene:reorder');
    }
    function setActiveScene(sceneId) { requireScene(sceneId); const next = getProject(); next.activeSceneId = sceneId; return validateAndCommit(next, 'scene:activate'); }
    ...
    function confirmBreakdown(draft = {}) {
      ...
      const next = getProject(); next.sceneOrder = []; next.scenes = {};
      draft.proposedScenes.forEach((proposal, index) => {
        const id = `scene-${String(index + 1).padStart(2, '0')}`;
        const record = createEmptySceneRecord({
          id, order:index+1, title:proposal.title,
          narrativeRole:{ role:proposal.role, narrativeFunction:proposal.narrativeFunction, startingState:proposal.startingState, endingState:proposal.endingState, turningPoint:proposal.turningPoint, agencyTransition:clone(proposal.agencyTransition), relationToPrevious:proposal.relationToPrevious ?? null }
        });
        const checked = validateSceneRecord(record);
        if (!checked.valid) throw new Error(`Invalid Scene Proposal ${index + 1}: ${checked.errors.join('; ')}`);
        next.sceneOrder.push(id); next.scenes[id] = checked.value;
      });
    ...
    function subscribe(listener) {
      ...
      listeners.add(listener); listener(getProject(), 'subscribe'); return () => listeners.delete(listener);
    }
    if (initial) {
    ...
    return { getProject, subscribe, createProject, updateProjectMetadata, setProjectConstraints, addScene, updateScene, removeScene, reorderScenes, setActiveScene, saveSceneSnapshot, confirmBreakdown };
  }
